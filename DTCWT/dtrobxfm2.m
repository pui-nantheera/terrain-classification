...
if nlevels >= 1,
   ...
   Lo = colrobfilter(X,h0o).';
   Hi = colrobfilter(X,h1o).';
   
   % Do odd top-level filters on rows.
   LoLo = colrobfilter(Lo,h0o).';			% LoLo
   Yh{1} = zeros([size(LoLo)/2  6]);
   Yh{1}(:,:,[1 6]) = q2c(colrobfilter(Hi,h0o).');			% Horizontal pair
   Yh{1}(:,:,[3 4]) = q2c(colrobfilter(Lo,h1o).');			% Vertical pair
   Yh{1}(:,:,[2 5]) = q2c(colrobfilter(Hi,h1o).');	      % Diagonal pair
   S = [ size(LoLo) ;S];
   if nargout == 3
      Yscale{1} = LoLo;
   end
...
if nlevels >= 2;
   for level = 2:nlevels;
      ...
      Lo = colrobdfilt(LoLo,h0b,h0a).';
      Hi = colrobdfilt(LoLo,h1b,h1a).';
      
      % Do even Qshift filters on columns.
      LoLo = colrobdfilt(Lo,h0b,h0a).';	%LoLo
      Yh{level} = zeros([size(LoLo)/2  6]);
      Yh{level}(:,:,[1 6]) = q2c(colrobdfilt(Hi,h0b,h0a).');	% Horizontal
      Yh{level}(:,:,[3 4]) = q2c(colrobdfilt(Lo,h1b,h1a).');	% Vertical
      Yh{level}(:,:,[2 5]) = q2c(colrobdfilt(Hi,h1b,h1a).');	% Diagonal   
      S = [ size(LoLo) ;S];
      if nargout == 3
         Yscale{level} = LoLo;
      end
