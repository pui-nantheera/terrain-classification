...
for k = 1:length(angleRange)
    ...
    % record results
    name{k} = [rootDir,kernelType,'LAVA',num2str(resizeRatio),'_SFTAlevel',num2str(numlevels),blurType,'_',num2str(angle),'.txt'];
    fileID{k} = fopen(name{k},'a');
    if  isempty(blurType)
        fprintf(fileID{k},'%3s  %7s  %7s\n', 'pb#', 'train', 'test');
    else
        fprintf(fileID{k},'%4s  ', 'blur');
        for bi = startprob:lastprob
...
% -------------------------------------------------------------------------
savename =  [rootDir,'LAVA',num2str(resizeRatio),'_SFTAlevel',num2str(numlevels),'.mat'];
if exist(savename,'file')==0
    ...
    for cnum = 1:25
        for k = 1:40
            ...
            allFeatures{cnum,k} =  sfta(img,numlevels);
        end
    end
...
for n = 1:length(angleRange)
    ...
    for numradius = 1:totalBlurRange
        ...
        if radius==0
            ...
            savename =  [rootDir,'LAVA',num2str(resizeRatio),kernelType,'_SFTAlevel',num2str(numlevels),...
            ...
            if exist(savename,'file')==0
                ...
                for cnum = 1:25
                    for k = 1:40
                        ...
                        allFeaturesblur{cnum,k} = sfta(img,numlevels);
                    end
                end
