...
for k = 1:length(angleRange)
    ...
    % record results
    fileID{k} = fopen([rootDir,kernelType,'Outex_TC_00000LBP_',blurType,'_',num2str(angle),'.txt'],'a');
    if  isempty(blurType)
...
for probcase = 0:99
    ...
    % extract feature
    trainingFeatures = zeros(totalTrain,59);
    for k = 1:totalTrain
        % read image
        img = imread([rootDir,'images\',names{k+1}]);
        
        trainingFeatures(k,:) = findLBPhist(img);
    ...
    for n = 1:length(angleRange)
        ...
        for numradius = 1:totalBlurRange
            ...
            % extract feature
            testingFeatures = zeros(totalTest,59);
            for k = 1:totalTrain
                % read image
                img = imread([rootDir,'images\',names{k+1}]);
                % blur image
                ...
                testingFeatures(k,:) = findLBPhist(img);
            end
        ...
        if strcmpi(blurType,'motion')
            fprintf(fileID{n},'%03d  %.4f  %.4f  %.4f  %.4f  %.4f  %.4f  %.4f  %.4f  %.4f  %.4f  %.4f\n', probcase, accTrain, recordAcc{n}(probcase+1,:));
        else
            fprintf(fileID{n},'%03d  %.4f  %.4f  %.4f  %.4f  %.4f\n', probcase, accTrain, recordAcc{n}(probcase+1,:));
        end
    end
