...
for testnum = 1:length(testType)
    ...
    for it = 1:numIteration
        ...
        for k = 1:maxNumTest
            ...
            ens = fitensemble(trainingDataCur,trainingLabels,'AdaBoostM1',100,'tree');
            ...
            % prediction process
            predictLabels = predict(ens,testingDataCur);
            accBoostCur = mean(predictLabels==testingLabels)*100;
            
            % record results
            % -------------------------------------------------------------------------
            computeTimeAll(it,k) = toc;
            accBoostAll(it,k) = accBoostCur;
            
        end
    end
    fprintf('\n');
    computeTime(testnum,:) = mean(computeTimeAll);
    accBoost(testnum,:) = mean(accBoostAll);
