function histB = binaryUDTCWTdenoise(img,wlevels,startlevel,numfeatures,includeavg,mask)
% histB = binaryUDTCWTdenoise(img,wlevels,startlevel,mask)
%           Find histogram of binary phase of undecimated DT-CWT (UDT-CWT)
%           and denoising with bivariance shrinkage
%       inputs:
...
%
%   v1.0 19-02-14 Pui Anantrasirichai, University of Bristol
...
for c = 1:2
    for d = 1:3
        ...
        for level = startlevel:wlevels
            currentBand = w{level}{1}{c}{d} + 1i* w{level}{2}{c}{d};
            if level < wlevels
                parentBand = w{level+1}{1}{c}{d} + 1i* w{level+1}{2}{c}{d};
            else
                parentBand = w{level}{1}{c}{d} + 1i* w{level}{2}{c}{d};
            end
            % Signal variance estimation
            Wsig = conv2(windowfilt,windowfilt,w{level}{1}{c}{d}.^2,'same');
            ...
            % add to bit-planes
            bitplanes = bitplanes + (real(denoiseBand)>0)*(2^(2*(level-startlevel)));
            bitplanes = bitplanes + (imag(denoiseBand) >0)*(2^(2*(level-startlevel)+1));
        end
        % histogram
        if (nargin >= 6)
