function [Yl,Yh,Yscale] = dtwavexfm2overcompleteDecorr(X,nlevels,biort,qshift)
...
if nlevels >= 2;
   for level = 2:nlevels;
      ...
      Yh{level}(:,:,[1 6]) = q2c(coldfiltovercomplete(Hi,h0b,h0a).');	% Horizontal
      Yh{level}(:,:,[3 4]) = q2c(coldfiltovercomplete(Lo,h1b,h1a).');	% Vertical
      Yh{level}(:,:,[2 5]) = q2c(coldfiltovercomplete(Hi,h1b,h1a).');	% Diagonal   
      S = [ size(LoLo) ;S];
      if nargout == 3
         Yscale{level} = LoLo;
      end
...
dd = dist(pp,pp');
C = rho.^dd;
Yh{level}(:,:,[1 6]) = q2c(colfilter(Hi,h0o).');			% Horizontal pair
   Yh{level}(:,:,[3 4]) = q2c(colfilter(Lo,h1o).');			% Vertical pair
   Yh{level}(:,:,[2 5]) = q2c(colfilter(Hi,h1o).');	      % Diagonal pair
