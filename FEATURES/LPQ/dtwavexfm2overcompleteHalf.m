function [Yl,Yh,Yscale] = dtwavexfm2overcomplete(X,nlevels,biort,qshift)
...
if nlevels >= 2;
   for level = 2:nlevels;
      ...
      if level > 2
          ...
          Lo = coldfiltovercomplete(LoLo,h0b,h0a).';
          Hi = coldfiltovercomplete(LoLo,h1b,h1a).';
          % Do even Qshift filters on columns.
          LoLo = coldfiltovercomplete(Lo,h0b,h0a).';	%LoLo
          Yh{level} = zeros([size(LoLo)  6]);
          Yh{level}(:,:,[1 6]) = q2covercomplete(coldfiltovercomplete(Hi,h0b,h0a).');	% Horizontal
          Yh{level}(:,:,[3 4]) = q2covercomplete(coldfiltovercomplete(Lo,h1b,h1a).');	% Vertical
          Yh{level}(:,:,[2 5]) = q2covercomplete(coldfiltovercomplete(Hi,h1b,h1a).');	% Diagonal
      else
          % Do even Qshift filters on rows.
          ...
          Yh{level} = zeros([size(LoLo)  6]);
          Yh{level}(:,:,[1 6]) = q2covercomplete(coldfilt(Hi,h0b,h0a).');	% Horizontal
          Yh{level}(:,:,[3 4]) = q2covercomplete(coldfilt(Lo,h1b,h1a).');	% Vertical
          Yh{level}(:,:,[2 5]) = q2covercomplete(coldfilt(Hi,h1b,h1a).');	% Diagonal
      end
...
function z = q2covercomplete(y)

% function z = q2c(y)
% Convert from quads in y to complex numbers in z.

sy = size(y);
t1 = 1:sy(1)-1; t2 = 1:sy(2)-1;
j2 = sqrt([0.5 -0.5]);
