function [LPQdesc LPQdescImg] =ri_lpq(img,LPQfilters,charOri,mode,mask)
...
%     LPQdescImg = zeros(imgRow, imgCol);
%     LPQdescImg(r+1:end-r,r+1:end-r) = reshape(LPQdesc,[(imgRow-winSize+1),(imgCol-winSize+1)]);
quantisecrop = reshape(LPQdesc,[(imgRow-winSize+1),(imgCol-winSize+1)]);
LPQdescImg = zeros(imgRow,imgCol);
LPQdescImg(r+1:end-r,r+1:end-r) = quantisecrop;
LPQdescImg([1:r end-r+1:end],:) = LPQdescImg([2*r:-1:r+1 end-1:-1:end-r],:);
LPQdescImg(:,[1:r end-r+1:end]) = LPQdescImg(:,[2*r:-1:r+1 end-1:-1:end-r]);
% end
...
if strcmp(mode,'nh') || strcmp(mode,'h')
    if nargin < 5
        ...
        mask = mask(r+1:end-r,r+1:end-r);
        LPQdesc=hist(LPQdesc(mask(:)>0),0:255);
    end
end
